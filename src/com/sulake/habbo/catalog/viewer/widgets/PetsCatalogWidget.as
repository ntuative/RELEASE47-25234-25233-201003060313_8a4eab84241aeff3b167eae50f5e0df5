package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_401 >= var_303.length)
         {
            return "";
         }
         var petColor:IPetColor = var_303[var_401] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_561[var_1455] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_401 = param1.index;
         if(var_401 < 0 || var_401 > var_303.length)
         {
            var_401 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_607.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         var_824 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_303 = [];
         var_561 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_824);
               if(_loc11_)
               {
                  var_561 = _loc11_.breeds;
                  var_303 = _loc11_.colors;
                  if(var_303 == null)
                  {
                     var_303 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_561.length)
         {
            _loc12_ = var_561[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_824,_loc12_.id),getRaceLocalizationKey(var_824,_loc12_.id)));
            _loc6_++;
         }
         var_607.add(var_824,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_808,onApproveNameResult);
         _loc5_ = var_607.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_303)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_607.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_401 >= 0 && var_401 < var_303.length)
         {
            _loc7_ = var_303[var_401] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_561[var_1455] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_824,_loc3_.id,_loc2_,AvatarScaleType.const_53);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_333,AvatarAction.const_540);
