package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      public const const_1653:int = 3;
      
      public const const_1587:int = 2;
      ...
      private var var_1658:MarketplaceItemStats;
      ...
      {
         if(!var_147 || !_catalog || !_catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = var_147.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         showConfirmation(const_1564,_loc2_);
      }
      ...
      {
         if(param1.furniCategoryId != var_1659 || param1.furniTypeId != var_1660)
         {
            return;
         }
         var_1658 = param1;
      ...
      public function onCancelResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            item = var_184.remove(parser.offerId);
            if(item != null)
            {
               item.dispose();
            }
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = var_147.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = var_147.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               var_147.add(parser.offerId,updateItem);
            }
            var_147.remove(parser.requestedOfferId);
            showConfirmation(const_1587,updateItem);
            _visualization.listUpdatedNotify();
         }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = var_184.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = var_184.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_419)
            {
               var_184.remove(_loc2_);
               _loc3_.dispose();
            }
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         var_1662 = _loc3_.creditsWaiting;
         var_184 = new Map();
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_184.add(_loc4_.offerId,_loc6_);
         }
      ...
      private function showConfirmation(param1:int, param2:MarketPlaceOfferData) : void
      {
         if(!var_189)
         {
            var_189 = new MarketplaceConfirmationDialog(this,_catalog,_roomEngine);
         }
         var_189.showConfirmation(param1,param2);
      }
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            var_147.add(_loc4_.offerId,_loc6_);
         }
         if(_visualization == null)
         {
            return;
      ...
      {
         requestOffers(var_1666,var_1664,var_1661,var_1665);
      }
