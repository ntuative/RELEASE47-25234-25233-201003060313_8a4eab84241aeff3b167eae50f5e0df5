package com.sulake.habbo.avatar.actions
{
   ...
   public class AvatarActionManager
   {
      ...
      private var var_204:Dictionary;
      ...
      public function AvatarActionManager(param1:IAssetLibrary, param2:String)
      {
         ...
         super();
         var_107 = param1;
         var_204 = new Dictionary();
         var_743 = param2;
         switch(param2)
         {
            case AvatarType.const_80:
               if(!var_107.hasAsset("HabboAvatarActions"))
               {
                  Logger.log("[AvatarStructure] Could not init human Action Data.");
               }
               else
               {
                  _loc3_ = var_107.getAssetByName("HabboAvatarActions").content as XML;
               }
               break;
            case AvatarType.PET:
               if(!var_107.hasAsset("HabboAvatarPetActions"))
               {
                  Logger.log("[AvatarStructure] Could not init pet Action Data.");
               }
               else
               {
                  _loc3_ = var_107.getAssetByName("HabboAvatarPetActions").content as XML;
               }
         }
         if(_loc3_ == null)
         {
      ...
      private function filterActions(param1:Array) : Array
      {
         ...
         while(_loc6_ < param1.length)
         {
            ...
            _loc5_ = var_204[_loc4_.actionType] as ActionDefinition;
            if(_loc5_ != null)
            {
      ...
      public function sortActions(param1:Array) : Array
      {
         ...
         {
            _loc4_ = var_204[_loc3_.actionType];
            if(_loc4_ != null)
            {
      ...
      public function getCanvasOffsets(param1:Array, param2:String, param3:int) : Array
      {
         ...
         while(_loc7_ < param1.length)
         {
            ...
            _loc6_ = var_204[_loc5_.actionType] as ActionDefinition;
      ...
      private function parseActionOffsets() : void
      {
         ...
         var _loc10_:* = null;
         for each(_loc2_ in var_204)
         {
            _loc3_ = _loc2_.state;
            if(var_107.hasAsset("action_offset_" + _loc3_))
            {
               _loc4_ = var_107.getAssetByName("action_offset_" + _loc3_).content as XML;
               if(_loc4_.@avatartype == var_743)
               {
