package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_773:uint = 2;
       
      
      private var var_683:Error;
      
      protected var _disposed:Boolean = false;
      
      private var var_258:Dictionary;
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_773)
            {
               if(var_683 != null)
               {
                  Core.crash("Error catched in EventDispatcher: undefined",var_683.errorID,var_683);
               }
               else
               {
                  Core.crash("Error catched in EventDispatcher. No error data available!",0,var_683);
               }
            }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_258[event.type];
         if(array)
         {
            callbacks = [];
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1097) : uint(const_494);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_258)
            {
               _loc2_ = var_258[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete var_258[_loc1_];
            }
            var_258 = null;
