package com.sulake.core.window.utils
{
   ...
   public class WindowMouseOperator implements IDisposable
   {
      ...
      protected var var_528:Point;
      
      protected var var_120:DisplayObject;
      ...
      protected function handler(param1:Event) : void
      {
         ...
         if(_working)
         {
            if(param1.type == Event.ENTER_FRAME)
            {
               ...
               {
                  operate(var_120.mouseX,var_120.mouseY);
      ...
      public function operate(param1:int, param2:int) : void
      {
         _mouse.x = param1;
         _mouse.y = param2;
         getMousePositionRelativeTo(_window,_mouse,var_528);
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(_working)
         {
            if(_window == param1)
            {
               var_120.removeEventListener(MouseEvent.MOUSE_DOWN,handler,false);
               var_120.removeEventListener(MouseEvent.MOUSE_UP,handler,false);
               var_120.removeEventListener(Event.ENTER_FRAME,handler);
               if(true)
               {
                  _window.removeEventListener(WindowEvent.const_420,clientWindowDestroyed);
               }
               _window = null;
               _working = false;
            }
      ...
      public function begin(param1:IWindow, param2:uint = 0) : IWindow
      {
         ...
         {
            var_120.addEventListener(MouseEvent.MOUSE_DOWN,handler,false);
            var_120.addEventListener(MouseEvent.MOUSE_UP,handler,false);
            var_120.addEventListener(Event.ENTER_FRAME,handler);
            _mouse.x = var_120.mouseX;
            _mouse.y = var_120.mouseY;
            _window = WindowController(param1);
            getMousePositionRelativeTo(param1,_mouse,_offset);
            _window.addEventListener(WindowEvent.const_420,clientWindowDestroyed);
            _working = true;
         }
         return _loc3_;
      }
